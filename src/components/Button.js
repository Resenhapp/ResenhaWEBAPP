...
const Button = ({ label, icon, action, iconSide = 'left', height = 1, width = 1, textAlign = 'center', active = true }) => {
    ...
    const containerStyles = `bg-whiteT1 shadow-lg shadow-[#ffffff26] text-purpleT3 hover:bg-white font-bold py-5 px-4 rounded-full flex justify-center items-center ${sizes[width]} h-${sizes[height]} ${active ? "bg-whiteT1" : "deactivated"} max-w-[500px]`;

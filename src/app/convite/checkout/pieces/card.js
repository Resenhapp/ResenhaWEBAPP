            ...
            <section className="flex flex-col items-center w-full max-w-md">
                <div className="flex flex-col w-full">
                    <div className="flex flex-col mt-12 mb-2 gap-4 w-full">
                        ...
                        <ReactInputMask mask="9999 9999 9999 9999" maskChar="" value={number} onChange={handleNumberChange} >
                        {(inputProps) => <InputField placeholder="Número do cartão" value={number} action={handleNumberChange} showIcon={true} Icon="card" type='card'/> }
                    </ReactInputMask>
                        <div className='flex flex-row gap-4'>
                        <ReactInputMask mask="99/99" maskChar="" value={expiration} onChange={handleExpirationChange}>
                        {(inputProps) =>
                            <InputField placeholder="Validade" value={expiration} action={handleExpirationChange} showIcon={true} Icon="calendar" type='date'/>
                        }
                    </ReactInputMask>
                    <ReactInputMask mask="9999" maskChar="" value={cvv} onChange={handleCvvChange}>
                        {(inputProps) =>
                            <InputField placeholder="CVV" value={cvv} action={handleCvvChange} showIcon={true} Icon="cvv" />
                        }
                    </ReactInputMask>
                        </div>
                        <ReactInputMask mask="999.999.999-99" maskChar="" value={cpf} onChange={handleCpfChange}>
                        {(inputProps) =>
                ...
                <div className="justify-center align-center w-full max-w-screen-xs flex mb-8"></div>
            </section>
        </div>
    )
}